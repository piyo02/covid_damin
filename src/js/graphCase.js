$(document).ready(function() {
    $.ajax({
        ...
        results[0].forEach(data => {
            dataIndonesia.push(data.cumulative_cases);
        })

        results[1].forEach(data => {
            dataFilipina.push(data.cumulative_cases);
        })

        results[2].forEach(data => {
            dataMalaysia.push(data.cumulative_cases);
        })
        
        results[3].forEach(data => {
            dataSingapura.push(data.cumulative_cases);
        })
